...
\includepdf[pages={1}]{ituforside.pdf}
...
\begin{spacing}{1.2}
\begin{abstract}
In this thesis a framework for generating complete 2-dimensional games- and levels in the video game description language VGDL is presented.
The generator is able to automatically generate games, test the playability and quality using general game playing algorithms, and score each game according to a series of evaluation criteria.

The research and framework is separated into two parts, divided by different game genres: 
First an analysis of 2-dimensional action-arcade games, with the goal of generating complete games using the performance of general game-playing algorithms.
Secondly, an examination of 2-dimensional turn-based puzzle games-, and especially the levels of puzzle games, with the ambition of being able to not only generate simple sets of game-rules, but also to generate non-trivial puzzles (levels) for the games.

The results of this thesis show that automatically generating game design is indeed very promising, with a few interesting complete games generated.
The game generators constructed in this work only guarantee that generated games are playable by a certain set of parameters, and is not able to distinguish between ``not-too-bad'' and actual good games -- however they are definitely able to find games with a high proportion being of decent quality.
In the results of this work, a human designer is still needed to assess generated games to figure out if they are actually fun or not.
...
The methods have mainly been focusing on some specific game genres- or frameworks, for instance by generating complete board games \citep{pell1992metagame, browne2008automated}, card games \citep{font2013towards}, or generating simple 2-dimensional graphics games \citep{togelius2008experiment, smith2010variations}.
...
%The first approach focused on using the results of a series of general game playing, knowledge free, algorithms playing through constructed VGDL games- and levels, and using the collection of their performance profiles to asses the quality of the played games.
%A series of human-designed games, assumed to be human-enjoyable, was used as a baseline for ``good games'' and was routinely used in choosing and weighing features used for the evolutionary strategies.


%For both methods, a series of human-designed games, assumed to be human-enjoyable, was used as a baseline for ``good games'' and was routinely used in choosing and weighing features used for the evolutionary strategies.
...
{4. Analysis of generated games};
\node[chapterheader]   (task2h)  [right=of task1h, shift = (right:0.0cm)]		
{5. Evaluation and evolution of games};
...
%In addition to the other game genres mentioned above, a lot of work has been done in generating levels for puzzle games (ie. Soko-Ban and Cut The Rope).
...
The arguably most successful attempt so far, Browne's Ludi system, managed to produce a new board game of sufficient quality to be sold as a boxed product~\citep{browne2008automated}. 
...
These measures are well-chosen for two-player board games, but might not transfer that well to video games or single-player games, which have in a separate analysis been deemed to be good targets for game generation~\citep{togelius2014characteristics}. 
Other researchers have attempted evaluation functions based on the learnability of the game by an algorithm~\citep{togelius2008experiment} or an earlier and more primitive version of the characteristic that is explored in this paper, performance profile of a set of algorithms~\citep{font2013towards}.
...
\begin{description}
	\item [SampleMCTS] Simulates the game space of possible actions within a small area of the games, using a ``Vanilla'' MCTS approach using UCT.
%	\item [GA] Uses a genetic mutation to evolve a sequence of actions.
...
A set of \textit{puzzle games} from several different platform, all featuring a single avatar, was found to be suitable to be described in VGDL. 
The games can mostly be described as \textit{Soko-Ban}-clones with different spins on the gameplay.
...
The games interpreted were \textit{Soko-Ban}, \textit{Bait}, \textit{Bombuzal}, \textit{Bolo Adventures}, \textit{Zen Puzzle}, \textit{The Citadel}, \textit{Brainman}, \textit{Chip's Challenge} and \textit{Modality}.
...
\label{fig:boloadventuresTranslation}
\end{figure}
...
\item[DeepSearch]
This controller is mix between a depth-first search- and a MCTS algorithm, seeking to advance deep into a tree with a few initial actions.
...
Also, the controller occasionally (every time a new path has been simulated to a length of 4) inputs the initial game state to be simulated from again, when other simulations have failed or have searched to a certain depth. 

\begin{algorithm}[H]
queue $\gets$ initial gameState \\
justExpandedFromInitial $\gets$ false \\
\While{has time left}{
	gameState = queue.poll() \\
	\eIf{gameState == initialGameState}{
		\For{action : PossibleActions}{
			queue $\gets$ gameState.copy().advance(action)
		}
		justExpandedFromInitial $\gets$ false
	}{
		queue $\gets$ gameState.advance(random, but not opposite to first-, action) \\
		d $\gets$ depth of search //amount actions performed \\
		value[action] $+=$ value(newGameState) $/ \text{ PossibleActions.length}^{\text{d}}$
		
		\If{d == 4 \textbf{and} !justExpandedFromInitial}{
			queue $\gets$ initial gameState \\
			justExpandedFromInitial $\gets$ true
		}
	}
}
\Return action leading to highest value, or random action if values are equal
...
Unlike the other controllers which only use the current state to decide on an action, it stores information about visited tiles and prefers visiting unvisited locations. 
...
Below is a shortened (since the controller is rather complex) explanation of the algorithms approach:

\begin{algorithm}[H]
queue $\gets$ initial gameState \\
deathActions, scoreActions, boredActions $\gets$ [] \\
\While{has time left}{
	gameState = queue.poll() \\
	\eIf{gameState == initialGameState}{
		\For{action : PossibleActions}{
		...
		d $\gets$ depth of search //amount actions performed \\
		firstAct $\gets$ first action performed \\
		deathActions[firstAct] $+=$ isDead(gameState) $/ \text{ PossibleActions.length}^{\text{d}}$ \\
		scoreActions[firstAct] $+=$ scoreValue(gameState) $/ \text{ PossibleActions.length}^{\text{d}}$ \\
		boredActions[firstAct] $+=$ boredValue(gameState) \\
		queue $\gets$ gameState.advance(with least boring action) \\
	}	
}

\If{deathActions values higher than threshold}{
	\Return action for lowest deathAction value
}
\If{scoreActions difference higher than threshold}{
	\Return action for highest scoreActions value
}
\Return action for lowest boredActions value
\caption{Explorer algorithm}
\end{algorithm}
...
visitedStates $\gets$ \{\} \\
solution $\gets$ [] \\
\While{has not found path leading to win}{
	gameState = queue.poll() \\
	
	\If{gameState.won == true}{
		solution $\gets$ path leading to win\\
		\textbf{break}
	}
	\While{gameState contains Flickers}{
		gameState.advance(NIL action)
	}
	\If{!visitedStates.contains(signatureOf(gameState))}{
		\For{action : PossibleActions}{
			\If{\textbf{not} action leads to wall position}{
				queue $\gets$ gameState.copy().advance(action)
			}
		}
	}
		
	visitedStates.add(signatureOf(gameState))
}

\Return solution[idx++]
\caption{PuzzleSolver algorithm}
\end{algorithm}
...
%                "missilecommand", "portals", "Soko-Ban", "survivezombies", "zelda",
...
In the two approaches several constraints were set upon the generation process, both to prevent crashes and to limit parameters to values known to be good in the designed games\footnote{The set constraints used when generating game descriptions were rather intricate, and difficult (while not \textit{that} interesting) to present in its entirety. Curious readers are recommended to examine the source code for the process at: https://github.com/torybash/GameChanger/tree/master/src/gamechanger/core}, causing a slight decrease in the space of potential generated games.
...
I continuously tried evolving a set of weights for individual features, to balance the contribution from each type of values using a CMA-ES algorithm~\citep{hansen2003reducing}.
...
The evolution process was stopped when the highest fitness stopped increasing over 10 generation, when the best games fitness were above 0.98 (an almost perfect score), or when the 15 generations allowed were over.
...
\end{vgdldesc}
\caption{The VGDL game description for the evolved game \texttt{evol_game001}}
...
For instance, one could examine how each controller increased its score across a playthrough, th proportion of sprites- and rules that have been in use in the game, the amount of sprites that has left the level field, or the amount of objects that have been killed or created\footnote{I have implemented functions for retrieving several more value (like these) in the GVG-AI framework, but initial examination of the values did not show any clear form or profile, for the human-designed set of games: https://docs.google.com/spreadsheets/d/1HAuIdXBcv4MXTlerkSqz8tK-zXMypzzQAU8sRlt95TA/edit?usp=sharing}.

Additionally, a common problem for the generated games were a partly- or completely random outcome.
This problem seems to rather difficult to solve completely without actually reading the rules of the being played -- even if algorithms have played through a generated game an enormous amount of times it can not guarantee that the players actions are responsible for winning, or making a high score.
...
The work of this chapter was greatly inspired by research into puzzle game generation~\citep{6932896} and puzzle level generation~\citep{williams2012gold, TaylorParberry2011TRSokoban}.
...
\begin{figure}[!ht]
	\subfloat{
      	\includegraphics[scale=0.34]{baitgenlvl.png}
    	}
	\hfill
	\subfloat{
      	\includegraphics[scale=0.34]{thecitadelgenlvl.png}
    	}\\
	\subfloat{
		\includegraphics[scale=0.34]{sokobangenlvl.png}
	}\hfill
	\subfloat{
      		\includegraphics[scale=0.34]{modalitygenlvl.png}
    	}\\
...
\begin{figure}[!ht]
	\subfloat{
      	\includegraphics[scale=0.55]{evolpuzzle2.png}
    	}
	\hfill
	\subfloat{
      	\includegraphics[scale=0.55]{evolpuzzle1.png}
    	}\\
...
\section{My work}
The goal of this project was to create a process for creating games and levels in the game description language VGDL being ``as-enjoyable-as-possible'' .
To reach this goal, two different approaches was attempted:

Firstly, generating the VGDL game descriptions of \textit{action-arcade games}, examining if it was possible to use the performance of general game playing algorithms to assess the games' quality.
The method was successful at generating interesting game designs, and indicates that using the performance of game playing algorithms to generate content is worth examining further: 
...
And secondly, generating VGDL descriptions for \textit{puzzle games}, using the performance of a breadth-first search algorithm to assess the difficulty, and thereby the quality of a game, -and importantly, a level.
This approach indicates that the shortest solution for a \textit{puzzle} level, can be used assess the difficulty (which is probably loosely proportional to the quality) of games- and levels of the game-genre.
...
%1) Generating general game description -- \textit{action-arcade} games, using all the possible classes and functions of VGDL.
%And 2) Generating game descriptions restricted to a subset of VGDL classes -- \textit{puzzle games}.
...
%, or even using controllers designed to ``play bad'' (trying to die, decrease score) could be interesting

% might also be interesting to analyse games using an even wider array of controllers, or using controllers with a range of different time allowed per frame.
...
%\item [Painter] Very simple game, with a very limited skill ceiling. The player must make every tile in levels have the same colour, which is achieved by moving the avatar in a path across each tiles, causing each to switch colour.
