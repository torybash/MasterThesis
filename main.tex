...
A second, smaller, part of this thesis was focused on games of the \textit{puzzle}-genre describable, in VGDL.
...
The VGDL puzzle games examined consist in general of a much smaller game-state space (i.e. the amount of possible arrangements of sprites and values), and so potentially are more well-suited for a in-depth analysis.
...
In this thesis I make the simple distinction between the two types of games in the VGDL framework, that arcade games can contain elements (sprites) that move by themselves, whereas interactions only occur as a result of the avatar moving in puzzle games.
...
\subsubsection*{Games} 
From the GVG-AI framework (Section \ref{sec_gvgaiframework}) and my own additions (Section \ref{sec_writingnewvgdl}), there was a total of 9 puzzle games to use in the following analysis.
...
Because of this, only the following five games were used as a base-line for ``good'' puzzle games: Bait, Brainman, The Citadel, Soko-Ban and Modality.
...
\begin{figure}[!ht]
	...
	\subfloat{
    	...
    	\caption{Generated levels for some of the designed puzzle games. From top left: \textit{Bait} (shortest solution is in 65 actions), \textit{The Citadel} (77 actions), \textit{(Real) Soko-Ban} (60 actions) and \textit{Modality} (88 actions).}
...
\subsection{Results} 
Over a period of about 10 days, the generator was able to generate a long series of VGDL puzzle game description, each with a 8x8 level to play through the game with.
...
\end{vgdldesc}
\caption{The game description of the generated puzzle game \texttt{evol_puzzle015}}
...
As mentioned, the tight restrictions of my definition of VGDL puzzle games, and the process the evolution strategy use (requiring that a sprite has been moved), causes the actual games- and levels generated to have strong similarities.
Several of puzzle games generated definitely contain interesting features, but if any of the games can be considered of a high quality enough to be enjoyable for human players is a question left unanswered.
...
1) Focusing on a general descriptions allowed of the VGDL language,  
...
The main focus was on creating the VGDL game descriptions of \textit{action-arcade} games by using the performance of general game playing algorithms to assess the games' quality.
...
The process was able to generate several set of interesting game-rules, but at the same time several of the games created by the generator contained severely trivial game design, and in many games the outcome was largely based on luck.
In general the generator is not able to instantly find the most interesting games in the space of VGDL games, -- instead it is able to find a subset of more interesting game description, which human game designers can examine further to decide which games are actually ``good''.
